...
# ------------------------- Master Passphrase Functions -------------------------

def check_master_passphrase_set(conn: sqlite3.Connection) -> bool:
    """Check if master passphrase is already set"""
    c = conn.cursor()
    c.execute('SELECT COUNT(*) FROM master_key_meta WHERE id = 1')
...
class App(tk.Tk):
    def __init__(self, conn: sqlite3.Connection):
        ...
        self.master_passphrase_set = check_master_passphrase_set(self.conn)
    ...
    def build_login_screen(self):
        ...
        def do_login():
            ...
            if ok:
                ...
                # Check and setup master passphrase on first admin login if not set
                if u == 'admin' and not self.master_passphrase_set:
                    response = messagebox.askyesno(
                        "Setup Master Passphrase",
                        "Master encryption passphrase is not set.\n\n"
                        "This is required for file encryption/decryption.\n\n"
                        "Setup master passphrase now?"
                    )
                    if response:
                        self.setup_master_passphrase_dialog()
    ...
    def setup_master_passphrase_dialog(self):
        """Show dialog to setup master passphrase"""
        dialog = tk.Toplevel(self)
        dialog.title("Master Encryption Passphrase Setup")
        ...
        # Instructions
        instructions = """=== MASTER ENCRYPTION PASSPHRASE SETUP ===

This passphrase will be used to encrypt and decrypt all your files.
...
• Store it securely (password manager recommended)"""
        ...
        ttk.Label(frame, text=instructions, justify='left', wraplength=450).pack(pady=(0, 15))
        
        # Passphrase entry
        ttk.Label(frame, text="Enter Master Passphrase:").pack(anchor='w')
        entry1 = ttk.Entry(frame, show="*", width=50)
        entry1.pack(fill='x', pady=(0, 10))
        
        ttk.Label(frame, text="Confirm Master Passphrase:").pack(anchor='w')
        entry2 = ttk.Entry(frame, show="*", width=50)
        entry2.pack(fill='x', pady=(0, 10))
        
        # Strength indicator
        strength_var = tk.StringVar(value="Strength: Not checked")
        strength_label = ttk.Label(frame, textvariable=strength_var, foreground="gray")
        ...
        def check_strength(event=None):
            ...
            if ok:
                ...
                strength_label.configure(foreground="green")
            else:
                strength_var.set(f"✗ {msg}")
                strength_label.configure(foreground="red")
        
        entry1.bind('<KeyRelease>', check_strength)
        ...
        def do_setup():
            ...
            if not p1 or not p2:
                messagebox.showerror("Error", "Please enter passphrase in both fields", parent=dialog)
                return
            
            if p1 != p2:
                messagebox.showerror("Error", "Passphrases do not match", parent=dialog)
                return
            
            ok, msg = password_strength_ok(p1)
            if not ok:
                messagebox.showerror("Weak Passphrase", f"Passphrase is too weak:\n{msg}", parent=dialog)
                return
            
            # Setup the passphrase
            setup_master_passphrase_db(p1, self.conn)
            self.master_passphrase_set = True
            
            # Show success and reminder
            reminder = """✓ MASTER PASSPHRASE SETUP COMPLETE

IMPORTANT REMINDER:
1. Your master passphrase is now configured
2. You will need this passphrase to:
   • Encrypt new files
   • Decrypt existing files
3. If you forget this passphrase:
...
Click OK to continue."""
            ...
            messagebox.showinfo("Setup Complete", reminder, parent=dialog)
            dialog.destroy()
            return True
        
        def cancel():
            warning = """WARNING: Without master passphrase:

• You CANNOT encrypt files
• You CANNOT decrypt files
• File encryption features will be disabled

Are you sure you want to cancel setup?"""
            
            if messagebox.askyesno("Cancel Setup?", warning, parent=dialog):
                dialog.destroy()
        ...
        ttk.Button(btn_frame, text="Set Master Passphrase", 
                   command=do_setup, width=20).pack(side='right', padx=(10, 0))
        ttk.Button(btn_frame, text="Cancel", 
                   command=cancel, width=15).pack(side='right')
        
        # Focus and bind Enter key
        entry1.focus_set()
        dialog.bind('<Return>', lambda e: do_setup())
    ...
    def create_files_tab(self):
        ...
        status_text = "✓ Master passphrase configured" if self.master_passphrase_set else "⚠ Master passphrase NOT set"
        status_color = "green" if self.master_passphrase_set else "orange"
        self.files_status_label = ttk.Label(status_frame, text=status_text, foreground=status_color)
        self.files_status_label.pack(side='left')
        
        # Setup button if not configured
        if not self.master_passphrase_set:
            ttk.Button(status_frame, text="Setup Master Passphrase Now", 
                      command=self.setup_master_passphrase_now).pack(side='right')
        
        ttk.Button(top, text='Encrypt File', command=self.encrypt_file, 
                  state='normal' if self.master_passphrase_set else 'disabled').pack(side='left', padx=6)
        ttk.Button(top, text='Decrypt File', command=self.decrypt_file,
                  state='normal' if self.master_passphrase_set else 'disabled').pack(side='left', padx=6)
    ...
    def setup_master_passphrase_now(self):
        """Setup master passphrase from files tab"""
        self.setup_master_passphrase_dialog()
        if self.master_passphrase_set:
            # Update UI
            self.files_status_label.config(text="✓ Master passphrase configured", foreground="green")
            # Enable buttons
            for widget in self.tab_files.winfo_children()[0].winfo_children():
                if isinstance(widget, ttk.Button) and widget['text'] in ['Encrypt File', 'Decrypt File']:
                    widget.config(state='normal')
            messagebox.showinfo("Success", "Master passphrase configured. File encryption/decryption is now available.")

    def check_master_passphrase_before_operation(self, operation_type="encryption"):
        """Check if master passphrase is set before file operations"""
        if not self.master_passphrase_set:
            response = messagebox.askyesno(
                "Master Passphrase Required",
                f"Master passphrase is not set.\n\nYou need to set a master passphrase before you can perform {operation_type}.\n\nSetup master passphrase now?"
            )
            if response:
                self.setup_master_passphrase_now()
            return False
        return True

    def encrypt_file(self):
        # Check at the beginning
        if not self.check_master_passphrase_before_operation("encryption"):
            return
    ...
    def decrypt_file(self):
        # Check at the beginning
        if not self.check_master_passphrase_before_operation("decryption"):
            return
    ...
    def create_account_tab(self):
        ...
        if self.current_user == 'admin':
            status = "Configured" if self.master_passphrase_set else "Not configured"
            color = "green" if self.master_passphrase_set else "red"
            ttk.Label(frame, text=f'Master passphrase: {status}', foreground=color).pack(pady=2)
            
            if not self.master_passphrase_set:
                ttk.Button(frame, text='Setup Master Passphrase', 
                          command=self.setup_master_passphrase_now).pack(pady=4)
